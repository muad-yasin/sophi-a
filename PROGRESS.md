...
- [Phase 1, Step 2] Wizard UI and readiness gating — pass — real browser test (Tauri IPC + WebSocket stubbed, real message-handler path exercised) — 2026-09-10T20:52:47Z
- [Phase 3, Step 1] Run recorder with eviction — pass — real end-to-end run through `startRelayChainSeat` (fake relay CLI writing a real-shaped `report.json`/`run.log`) confirmed `runs/plan-1/<runId>/` gets both files; a 51-run script against `run-recorder.js` directly confirmed exactly 50 remain, oldest evicted — 2026-09-10T21:05:00Z
- [Phase 3, Step 2] Replay from history, honest by construction — pass — live WebSocket integration test (`list_runs`/`replay_run`/`get_seat_logs` against a running orchestrator) confirmed a replayed good run returns identical verdict/signoff/scoreboard text to its source `report.json`; a corrupted `report.json` and a nonexistent runId both return the literal `"unreadable report"` error, never an empty/partial reply — 2026-09-10T21:05:00Z
- [Phase 2, Step 3] Export a run as markdown — pass — `node scripts/verify-phase2-step3.mjs` (5/5 checks: Debate markdown carries every signoff/objection/scoreboard/verdict field verbatim, honest empty state, byte-exact disk round-trip, visible TRUNCATED marker on an oversized export, no truncation on a normal one) plus a real `npx tsc --noEmit` and `vite build` (both clean) and a real `cargo check` in src-tauri (clean, dist-node/dist-orchestrator symlinked in locally only to unblock the Tauri build-script's resource check - both are gitignored and not part of this change) — 2026-09-10T21:00:09Z
- [Phase 1, Step 3] Smoke run button — pass — real browser test (button DOM state: disabled red, enabled green, click dispatches correct {cmd:'start'}) — 2026-09-10T21:14:52Z
- [Phase 2, Step 2] Seat cost meter, real numbers only — pass — real standalone orchestrator + WS client driving a real relay `mock` chain run on `plan-1` (temporary seats.json override, reverted after), confirming `seat.usage`'s totals match relay's own printed `tokens:`/`cost:` line exactly, plus a real `advisor` call (`claude-fable-5-1`) showing `~2456 tokens` (unpriced, never a fabricated $) — 2026-09-10T21:05:00Z
- [Phase 2, Step 1] Stop-All + per-seat watchdog — pass — `node scripts/test-stopall-watchdog.mjs` (real orchestrator/WebSocket + real child-process spawn/kill against a temp-PATH fake `claude`, no mocking) — 2026-09-10T21:06:57Z
...
- [Backlog item 5] Interactive offline Council demo — pass — real Chrome tab against the running dev server: clicking "Try Council demo" on a plan-N tile renders the full 5-critic signoff matrix and lights up all five seal wedges, bearing a "SIMULATED DEMO — not a real run" banner, confirmed zero network requests fired during the click (network tracking active before and after); selecting "Live" afterward correctly returns to the honest "No relay run finished for this seat yet" empty state — 2026-09-11T00:22:36Z
